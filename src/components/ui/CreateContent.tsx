  ...
  const handleAiSuggest = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `${BACKEND_URL}/api/v1/ai/suggest`,
        { url: link, title: titleRef.current?.value.trim() || undefined },
              ...
              <div>
                ...
                {aiSuggestion && (
                  <p className="mt-1.5 text-xs text-purple-600 dark:text-purple-400">
                    {aiSuggestion.ai ? "✨ Powered by Gemini AI" : "⚡ Smart suggestions applied"}
                  </p>
                )}
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1.5 uppercase tracking-wide">
                  ...
                  {aiSuggestion?.summary && (
                    <span className="ml-2 normal-case font-normal text-purple-500">✨ AI summary</span>
                  )}
                </label>
                <textarea
                  value={notes}
